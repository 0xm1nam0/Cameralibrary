...
public class CameraSource implements Camera.PreviewCallback {
    ...
    private static class SingletonHolder {
        private static final CameraSource INSTANCE = new CameraSource();
    }
    ...
    public int openCamera(int direction) {
        if (camera != null) {
            closeCamera();
            LoggerUtils.i("open Camera: camera != null");
/*            camera.release();
            camera = null;*/
        }
        try {
            switch (direction) {
                ...
                case CAMERA_DIRECTION_FRONT:
                    camera = Camera.open(CAMERA_DIRECTION_BACK);
                    break;
                default:
                    camera = Camera.open();
                    break;
            ...
            LoggerUtils.i("open camera exception: "+e.toString());
            return -1;
        }
        return 0;
    }
    ...
    public List<Size> getSupportsSizes(){
        if(camera == null) {
            LoggerUtils.i("Camera getSupportsSizes: camera == null");
            return null;
        }
        LoggerUtils.i("Camera getSupportsSizes: camera != null");
