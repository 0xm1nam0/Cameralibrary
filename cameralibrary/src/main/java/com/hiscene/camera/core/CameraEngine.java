...
public class CameraEngine implements ICameraEngine {
    ...
    private int HEAD_CAMERA_ID;//G200 头戴端
    private int HAND_CAMERA_ID;//G200 手持端
    private int camId = 0;
    ...
    public CameraEngine(Context context, boolean isFront) {
        isFrontCamera = isFront;
        mContext = context;
        if (Constant.IS_G200) {
            boolean firmwareVersion = Constant.beyondFirmwareVersion1_0_0();
            if (firmwareVersion) {
                HEAD_CAMERA_ID = 0;
                HAND_CAMERA_ID = 1;
            } else {
    ...
    private int getCameraId(boolean isFront) {
        ...
        isFrontCamera = (camId == Camera.CameraInfo.CAMERA_FACING_FRONT);
        return camId;
    }

    private int getCameraIdForG200(boolean isFront) {
        camId = HAND_CAMERA_ID;
        int cameraNum = Camera.getNumberOfCameras();
        if (cameraNum > 1 && isFront) {
            camId = HEAD_CAMERA_ID;
        }
        return camId;
    }
    ...
    public synchronized int openCamera(boolean isFront, SurfaceTexture surfaceTexture) {
        inUse = true;
        LogUtils.i(TAG, "switchCamera: frontCamera %s,thread id=%s,thread name=%s",
                isFrontCamera, Thread.currentThread().getId(), Thread.currentThread().getName());
        try {
            if (null != camera) {
                closeCamera();
            }
            int index = getCameraId(isFrontCamera);
            ...
            LogUtils.i(TAG, "camera size: w-" + size.width + " h-" + size.height);
            previewCount = 0;
     ...
     */
    @Override
    public synchronized void handleFocusMetering(int centerX, int centerY, FocusCallback callback) {
    ...
    public boolean switchFlashLight(Context context) {
        LogUtils.d(TAG, "switchFlashLight status : %s", flashMode);
        if (camera != null) {
    ...
    public void closeFlashLight(Context context) {
        LogUtils.e(TAG, "closeFlashLight");
        if (camera != null) {
...
//            LogUtils.d(TAG, "PreviewCallback len=%s", data.length);
    ...
    private Camera.Size getBestSize(List<Camera.Size> sizes) {
        ...
        for (Camera.Size s : sizes) {
            LogUtils.i(TAG, "support size: w-" + s.width + " h-" + s.height);
    ...
    public synchronized void handleZoom(float ratio) {
        if (camera != null) {
            ...
            if (params.isZoomSupported()) {
                ...
                if (ratio > 0 && zoom < maxZoom) {
                    zoom++;
                } else if (ratio < 0 && zoom > 0) {
                    zoom--;
                }
                params.setZoom(zoom);
                camera.setParameters(params);
